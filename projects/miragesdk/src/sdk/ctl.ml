...
module Client = struct
  ...
  type t = Ctl_api.Reader.Ctl.t Capability.t

  let read t path =
    let proxy = new Ctl_api.Reader.Ctl.client t in
    ...
    Capability.call_for_value proxy#read req >|= function
  ...
  let write t path data =
    ...
    P.path_set_list p path |> ignore;
    P.data_set p data;
    Capability.call_for_value proxy#write req >|= function
    | Ok _ -> Ok ()
  ...
  let delete t path =
    ...
    let req, p = Capability.Request.create P.init_pointer in
    P.path_set_list p path |> ignore;
    Capability.call_for_value proxy#delete req >|= function
    | Ok _ -> Ok ()
